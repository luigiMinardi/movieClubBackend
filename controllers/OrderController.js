    ...
    try {
        Order.findOne({
            ...
            order
                ...
                : res.status(404).json({ msg: 'This order does not exist.' })
        );
    } catch (e) {
        res.status(400).json({
            msg: 'Something unexpected happened',
...
OrderController.deleteOrderById = (req, res) => {
    ...
    try {
        Order.findOne({
            ...
            if (!order) {
                res.status(404).json({
                    msg: `Order with id ${id} does not exists, you can't delete a phantom.`,
                });
                ...
                res.status(200).json({
                    msg: `Order with id ${id} was deleted.`,
                });
            } else {
                res.status(403).json({
                    msg: `You aren't allowed to delete the order ${id}.`,
                });
            }
        });
    } catch (error) {
