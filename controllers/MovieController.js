...
MovieController.getMovieById = (req, res) => {
    try {
        Movie.findByPk(req.params.pk).then((data) => {
            ...
            if (data.adult && user.age >= 18) {
                ...
                res.status(403).json({
                    msg: 'You need to have 18 years or more to access this movie.',
                });
            }
        });
    } catch (err) {
    ...
    Movie.findAll({
        ...
        .then((adultMovies) => {
            ...
            if (adultMovies != 0 && user.age >= 18) {
                ...
                res.status(403).json({
                    msg: 'You need to have 18 years or more to access this zone.',
                });
            } else {
        ...
        Movie.findOne({
            ...
            if (!moviesWithSameTitle) {
                Movie.create({
                    ...
                    .then((movie) => {
                        res.status(201).json({
                            msg: `${movie.title}, created!`,
                        });
                    })
                    .catch((err) =>
                        res.status(400).json({
                            msg: `The movie creation failed.`,
                            error: err,
                        })
                    );
            } else {
                res.status(400).json({
                    msg: `The movie with the title "${moviesWithSameTitle.title}" is already registered.`,
                });
            }
        });
    } catch (error) {
        res.status(500).json({
            msg: `Something unexpected happened while creating movie`,
            error: { name: error.name, message: error.message },
        });
    }
};
    ...
    try {
        if (data.title) {
            Movie.findOne({
                ...
                if (movie) {
                    res.status(409).json({
                        msg: 'The movie with this title already exist',
                    });
                } else {
                    Movie.update(data, {
                        where: { id: id },
    ...
    try {
        Movie.findOne({
            ...
            if (movie) {
                ...
                res.status(200).json({
                    msg: `Movie with id ${id} was deleted.`,
                });
            } else {
                res.status(404).json({
                    msg: `Movie with id ${id} does not exists, you can't delete a phantom.`,
                });
            }
        });
    } catch (error) {
    ...
    try {
        ...
        for (let j = 1; j <= pageNumber; j++) {
            let results = await axios.get(
                `https://api.themoviedb.org/3/discover/movie?api_key=${key}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${minMaxRoundedRandom(
                    1,
                    pageNumber
            ...
            for (let i = 0; i < numbOfResultsPerPageTMDB; i++) {
                ...
                Movie.create({
                    ...
                    image:
                        TMDBimgUrlRoot +
                        '/' +
                        results.data.results[i].poster_path,
                });
            }
        }
        res.send(
            `${pageNumber} pages have been clonated succesfully, with a max amount of ${500} films if 25 pages`
        );
    } catch (error) {
        res.status(500).json({
            msg: `Something unexpected happened while cloning movieDB data.`,
